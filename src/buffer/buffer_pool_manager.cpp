...
bool BufferPoolManager::AllocateFrame(bustub::frame_id_t *frame_id) {
  if (!free_list_.empty()) {
    *frame_id = free_list_.front();
    ...
    if (replacer_->Victim(frame_id)) {
      ...
      // all pages in buffer pool already in use
      return false;
    }
  }
  return true;
...
void BufferPoolManager::FlushAllPagesImpl() {
  ...
  std::lock_guard<std::mutex> lock(latch_);
  for (size_t i = 0; i < pool_size_; i++) {
    if (pages_[i].IsDirty() && pages_[i].page_id_ != INVALID_PAGE_ID) {
